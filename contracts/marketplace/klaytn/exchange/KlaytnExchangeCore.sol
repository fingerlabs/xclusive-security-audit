...
import "../token/KIP7/KIP7Basic.sol";
import "../token/KIP17/IKIP17.sol";
import "../definix/DefinixHolderList.sol";
import "../MarketplaceStorage.sol";
import "../sunmiya/IStakingSunmiya.sol";
...
contract KlaytnExchangeCore is Context {
  ...
  function calculateFee(uint fee, bool isActiveAdminAddress, SellOrder memory sellOrder) internal view returns (uint) {
    KlaytnMarketplaceStorage marketStorage = KlaytnMarketplaceStorage(storageAddress);
    
    address defininxHolderAddress = marketStorage.getDefinixHolderListAddress();
    address sunmiyaAddress = marketStorage.getSunmiyaAddress();
    address stakingSunmiyaAddress = marketStorage.getStakingSunmiyaAddress();

    bool isDiamondHolder = false;
    uint sunmiyaBalance = 0;
    uint stakingSunmiyaBalance = 0;

    uint diamondHolderFee = fee;
    uint sunmiyaHolderFee = fee;

    if (isActiveAdminAddress) {
      isDiamondHolder = DefinixHolderList(defininxHolderAddress).isDiamondHolder(sellOrder.creator);
      sunmiyaBalance = IKIP17(sunmiyaAddress).balanceOf(sellOrder.creator);
      stakingSunmiyaBalance = IStakingSunmiya(stakingSunmiyaAddress).getStakingCount(sellOrder.creator);
    } else {
      isDiamondHolder = DefinixHolderList(defininxHolderAddress).isDiamondHolder(sellOrder.maker);
      sunmiyaBalance = IKIP17(sunmiyaAddress).balanceOf(sellOrder.maker);
      stakingSunmiyaBalance = IStakingSunmiya(stakingSunmiyaAddress).getStakingCount(sellOrder.maker);
    }

    if (isDiamondHolder) {
      diamondHolderFee = SafeMath.sub(diamondHolderFee, 50);
    }

    uint totalSunmiyaBalance = SafeMath.add(sunmiyaBalance, stakingSunmiyaBalance);

    if (totalSunmiyaBalance >= 15) {
      sunmiyaHolderFee = SafeMath.sub(sunmiyaHolderFee, 50);
    } else if (totalSunmiyaBalance < 15 && totalSunmiyaBalance >= 15) {
      sunmiyaHolderFee = SafeMath.sub(sunmiyaHolderFee, 40);
    } else if (totalSunmiyaBalance < 9 && totalSunmiyaBalance >= 6) {
      sunmiyaHolderFee = SafeMath.sub(sunmiyaHolderFee, 30);
    } 

    uint finalFee = fee;

    if (diamondHolderFee < sunmiyaHolderFee) {
      finalFee = diamondHolderFee;
    } else {
      finalFee = sunmiyaHolderFee;
    }
  ...
  function tokenTransfer(SellOrder memory sellOrder, BuyOrder memory buyOrder) internal returns (uint, uint, uint) {
    ...
    if (sellOrder.paymentToken != address(0)) {
      require(msg.value == 0, "when KIP7 transfer, must be value is zero");
    ...
    if (feeAddress != address(0) && fee > 0) {
      fee = calculateFee(fee, isActiveAdminAddress, sellOrder);
    ...
    if (sellOrder.paymentToken == address(0)) {
      require(_msgValue() == buyOrder.price, "not mathed buy price with value");
      ...
      if (sellOrder.publishType == PublishType.Mint && isActiveAdminAddress) {
        ...
        sellOrder.creator.transfer(royaltyAmount);
      }

    } else {
      require(KIP7(sellOrder.paymentToken).transferFrom(buyOrder.maker, feeAddress, feeAmount));

      if (sellOrder.publishType == PublishType.Mint && isActiveAdminAddress) {
        require(KIP7(sellOrder.paymentToken).transferFrom(buyOrder.maker, sellOrder.creator, receiveAmount), "failed receive amount transfer to royaltyReceiver");
        require(KIP7(sellOrder.paymentToken).transferFrom(buyOrder.maker, sellOrder.royaltyReceiver, royaltyAmount), "failed royalty amount transfer to royaltyReceiver");
      } else {
        require(KIP7(sellOrder.paymentToken).transferFrom(buyOrder.maker, sellOrder.maker, receiveAmount));
        require(KIP7(sellOrder.paymentToken).transferFrom(buyOrder.maker, sellOrder.creator, royaltyAmount));
      }
    }
